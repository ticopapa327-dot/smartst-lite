...
SmartST Server 统一 LiveKit、呼叫、权限、token、HIS、文件和审计
SmartST OR Agent 管理手术室采集、PTZ、录像和本地设备恢复
Android 会议平板作为正式客户端
手机 H5 仅单向收看
冷灰蓝 HMI 医疗设备控制屏视觉
```
...
- 真实 LiveKit preflight：验证真实服务端环境变量、RoomService 和 subscribe-only token 边界。
...
- SmartST Server / SmartST OR Agent / Desktop Client 三包安装器和 Windows Service。
...
- SmartST Server 负责 LiveKit 服务、token、呼叫、HIS、录像索引、上传和审计；可安装在手术室电脑，也可安装在独立服务节点。
- SmartST OR Agent 负责 Windows 本地采集、Native Worker、编码、录像、PTZ、设备恢复；它不保存 LiveKit API secret。
- Desktop Client 只做 UI 和操作入口；UI 关闭不应导致 Server、OR Agent、录像或 LiveKit 服务退出。
...
3. [docs/deployment-package-split.md](docs/deployment-package-split.md)
4. [docs/livekit-desktop-surgery-teaching-architecture.md](docs/livekit-desktop-surgery-teaching-architecture.md)
5. [docs/livekit-native-media-worker-service-feasibility.md](docs/livekit-native-media-worker-service-feasibility.md)
6. [docs/ui-visual-style.md](docs/ui-visual-style.md)
7. [docs/livekit-desktop-surgery-teaching-development-plan.md](docs/livekit-desktop-surgery-teaching-development-plan.md)
8. [docs/usb-first-rearchitecture.md](docs/usb-first-rearchitecture.md)
