...
视捷UST 是面向 Windows 手术室终端的桌面版手术示教软件项目。当前主线只保留 USB-first 手术室工作台、LiveKit/SFU 互动、Native Media Worker、本机 LiveKit 联调、UST Server / OR Agent / Desktop Client 三包边界相关代码。
...
UST Server 负责呼叫、room、token、HIS、文件、上传和审计
UST OR Agent 负责手术室采集、PTZ、录像和本地设备恢复
...
- `server-poc` 仍是内存业务服务原型，不是带数据库、认证和审计的生产 UST Server。
