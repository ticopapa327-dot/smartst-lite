...
当前仓库是第一阶段 MVP：桌面客户端外壳、发起端/接收端主要界面、局域网 ONVIF 自动发现、RTSP 本地预览、本地配置持久化、基础日志系统已经实现；ONVIF 认证取流、远端推流、LiveKit 通话仍是 TODO，不会伪造成功状态。
...
4. 留空 RTSP 地址时，确认系统生成候选 RTSP 地址；海康常见格式为 `rtsp://user:pass@ip:554/Streaming/Channels/101`。
5. 确认本机可运行 `ffmpeg -version`，或将 `ffmpeg.exe` 放到 EXE 同目录。
6. 保存摄像机后观察主/辅画面是否开始本地预览；若失败，查看界面提示的 `ffmpeg-preview-*.log`。
