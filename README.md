...
当前仓库是第一阶段 MVP：桌面客户端外壳、发起端/接收端主要界面、局域网 ONVIF 自动发现、本地配置持久化、基础日志系统已经实现；ONVIF 认证取流、RTSP 播放/转码、LiveKit 通话仍是 TODO，不会伪造成功状态。
...
- 摄像机服务：ONVIF WS-Discovery 自动发现，预留 ONVIF GetStreamUri、RTSP 地址管理
...
4. 留空 RTSP 地址时，确认系统生成候选 RTSP 地址。
5. 设置其中一路为主画面，确认主/辅画面标签变化。
6. 点击“创建房间”，确认生成 `ST-XXXXXX` 房间号。
7. 点击“呼叫接收端”，确认进入本地呼叫状态且提示 LiveKit/信令 TODO。
8. 进入“示教接收端”，录入地址和房间号，点击“主动加入”，确认最近连接被保存。
9. 进入“设置”，修改机构名称、设备名称、日志目录，重启后确认配置仍存在。
10. 检查 `%APPDATA%\\SmartST Lite\\logs\\smartst-lite.log` 是否写入操作日志。
