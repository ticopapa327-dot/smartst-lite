...
SmartST Server
  livekit-server.exe
  smartst-business-service.exe 或 node server service
...
- `LIVEKIT_API_SECRET` 只能存在于 `SmartST Server` 的服务环境变量或 `C:\ProgramData\SmartST\server\` 受限配置中。
- `SmartST OR Agent` 只持有短期 token，不保存 LiveKit API secret。
- `SmartST Desktop Client` 只保存 Server URL、OR Agent URL 和 UI 配置，不保存 LiveKit API secret、HIS 凭据或 FTP 密码。
...
| SmartST OR Agent control | 4781 | TCP/HTTP | 本机优先 | 设备、采集、录像控制 |
