...
| WASAPI render 枚举和格式探测 | `media-worker:native:list-devices` 返回 `audioRender` 数组和 `diagnostics.wasapiRender.status`；`media-worker:native:audio-render-probe` 返回 render mix format 和 `renderClientStatus=not-opened`；该项不等同回放、loopback 或 AEC 通过 |
