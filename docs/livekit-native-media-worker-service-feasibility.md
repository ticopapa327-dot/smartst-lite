...
> 适用仓库：`<repo-root>`
> 编写日期：2026-06-05  
> 结论：该组合可行，但必须按三包职责拆开实现。UST Server 负责 LiveKit、业务服务、JWT、呼叫、权限、审计和可选 HIS/上传；UST OR Agent 负责 Windows 本地采集、Native Media Worker、PTZ、预览、录像和设备恢复；Desktop Client 只做 UI 和人工操作入口。不能把本地采集录像交给 LiveKit，也不能把业务权限交给客户端 UI。
...
UST Desktop Client
  -> UST Server API / WebSocket
  -> UST OR Agent API / 本机控制面
...
UST Server 是控制面和实时服务节点。它可以安装在手术室电脑，也可以安装在独立院内主机。它包含 LiveKit Server 和业务服务，但二者仍是独立进程。
...
UST OR Agent 是手术室电脑上的本地后端。Native Media Worker 是 OR Agent 管理的媒体执行进程，不建议长期由 Tauri UI 直接托管。媒体采集和编码可能崩溃、卡死或泄漏资源，独立后台服务更容易重启和隔离。
...
- 手术室工作台 UI、示教室工作台 UI。
- 调用 UST Server 发起/接受呼叫、获取短期 token、显示房间状态。
- 调用 UST OR Agent 做设备选择、预览、录像、PTZ 操作。
...
- `ust-media-worker.exe` 生成 1280x720 RGBA 测试视频帧和 48 kHz 单声道测试音频。
...
1. `ust-server-poc`：从 `server-poc` 演进，支持 room、token、call、LiveKit preflight 和服务端配置。
