...
> 编写日期：2026-06-05。  
> 结论先行：LiveKit 适合做实时互动、房间、权限、弱网传输和远端订阅；本地多路采集、PTZ 控制、长时间录像、文件管理、HIS 对接必须由 UST Server、UST OR Agent 和 Desktop Client 分工承担，不能全部塞进 LiveKit、Tauri UI 或前端 WebView。
...
- UST Server、UST OR Agent、Desktop Client 必须逻辑分离；三者可以同装在手术室电脑上，也可以拆到不同主机。
...
flowchart LR
  subgraph OR["手术室 Windows 电脑"]
    ...
    Mic["板载声卡 / USB 全向麦"]
    Agent["UST OR Agent：采集、预览、录像、PTZ"]
  ...
  subgraph Server["UST Server：可同装手术室电脑或独立主机"]
...
- 默认现场形态是一体机：`UST Server`、`UST OR Agent`、`Desktop Client` 同装在手术室 Windows 电脑上。
- 更规范的院内部署可以分机：`UST Server` 独立安装，手术室电脑只运行 OR Agent 和 Desktop Client。
- 角色安装、Windows Service、配置、端口和防火墙规则以 `docs/deployment-package-split.md` 为准。
- 跨院区或公网：必须使用 TLS、TURN、中继带宽评估和访问审计。

## 3. 客户端架构

当前仓库是 Tauri 2 + React + TypeScript + Rust。后续 Desktop Client 保留该 UI 技术栈，但本地采集、录像和设备恢复要下沉到 UST OR Agent；LiveKit、JWT、呼叫和手机观察端权限要下沉到 UST Server。
...
- 可交付版本：本地采集和录像下沉到 UST OR Agent / Native Media Worker，UI 只做控制和渲染。
