...
> 编写日期：2026-06-05。  
> 结论先行：LiveKit 适合做实时互动、房间、权限、弱网传输和远端订阅；本地多路采集、PTZ 控制、长时间录像、文件管理、HIS 对接必须由 SmartST Server、SmartST OR Agent 和 Desktop Client 分工承担，不能全部塞进 LiveKit、Tauri UI 或前端 WebView。
...
- SmartST Server、SmartST OR Agent、Desktop Client 必须逻辑分离；三者可以同装在手术室电脑上，也可以拆到不同主机。
- 手术室端必须即使远程互动失败也能完成本地预览和本地录像；Desktop Client 关闭不能直接结束本地录像或服务端房间能力。
...
```mermaid
flowchart LR
  subgraph OR["手术室 Windows 电脑"]
    UVC["USB 摄像机 / USB 采集卡"]
    PTZ["USB / 串口 / 厂商 SDK PTZ 控制"]
    Mic["板载声卡 / USB 全向麦"]
    Agent["SmartST OR Agent：采集、预览、录像、PTZ"]
    Worker["Native Media Worker"]
    ORUI["Desktop Client：手术室工作台 UI"]
    UVC --> Agent
    Mic --> Agent
    PTZ --> Agent
    Agent --> Worker
  ...
  subgraph Server["SmartST Server：可同装手术室电脑或独立主机"]
...
- 默认现场形态是一体机：`SmartST Server`、`SmartST OR Agent`、`Desktop Client` 同装在手术室 Windows 电脑上。
- 更规范的院内部署可以分机：`SmartST Server` 独立安装，手术室电脑只运行 OR Agent 和 Desktop Client。
- 角色安装、Windows Service、配置、端口和防火墙规则以 `docs/deployment-package-split.md` 为准。
- 跨院区或公网：必须使用 TLS、TURN、中继带宽评估和访问审计。

## 3. 客户端架构

当前仓库是 Tauri 2 + React + TypeScript + Rust。后续 Desktop Client 保留该 UI 技术栈，但本地采集、录像和设备恢复要下沉到 SmartST OR Agent；LiveKit、JWT、呼叫和手机观察端权限要下沉到 SmartST Server。
...
SmartST Desktop Client
  ...
  Realtime Layer
    livekit-client
    房间连接、订阅轨道、交互模式下发布本地音频或后续指定轨道、Data/RPC、网络质量状态

  API Client
    SmartST Server API
    SmartST OR Agent API
    不保存 LiveKit API secret

SmartST OR Agent
  Media Control
    Native Media Worker 生命周期
...
SmartST Server
  Control Plane
    呼叫、room、token、权限、HIS、录像索引、上传、审计
    LiveKit Server 管理和 preflight
```
...
- 可交付版本：本地采集和录像下沉到 SmartST OR Agent / Native Media Worker，UI 只做控制和渲染。
