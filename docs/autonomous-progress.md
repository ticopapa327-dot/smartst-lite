...
- 验证：
  ...
  - video PGM export 阶段新增 Native Worker 命令 `exportVideoPayloadQueuePgm` 和脚本 `npm run media-worker:native:video-pgm-export`：通过，1000ms 内当前 1 路 `HD Webcam` 复制 8 帧 NV12 payload，native-side 导出 1 帧 Y 平面为 `native-worker/.tmp/video-payload-export.pgm`，`width=1280`、`height=720`、`fileBytes=921616`、`luma.min=2`、`luma.max=30`、`consumerStatus=pgm-export`、`exportedOverJson=false`。
  - video PPM export 阶段新增 Native Worker 命令 `exportVideoPayloadQueuePpm` 和脚本 `npm run media-worker:native:video-ppm-export`：通过，1000ms 内当前 1 路 `HD Webcam` 复制 8 帧 NV12 payload，native-side CPU 转换并导出 1 帧 P6 RGB PPM，`width=1280`、`height=720`、`fileBytes=2764816`、`rgb.r.max=35`、`rgb.g.max=14`、`rgb.b.max=20`、`consumerStatus=ppm-export`、`exportedOverJson=false`。
  ...
  - audio call drain 阶段新增 `npm run media-worker:native:audio-call-drain`：通过，2 秒内按 250ms 间隔模拟通话或 publisher 消费者 drain 5 个 PCM packet，8 次 drain 均消费到 packet，`totalConsumedPackets=40`、`totalConsumedBytes=153600`、`firstSequence=5`、`lastSequence=157`、sequence 单调递增、`finalConsumerStatus=manual-drain`、`finalQueueDepth=45`、`audioLevelStatus=measured`、`exportedOverJson=false`。
  ...
  - interaction drain 阶段新增 `npm run media-worker:native:interaction-drain`：通过，同一 Native Worker session 内启动 1 路视频和 1 路音频，2 秒内按 250ms 间隔同时 drain；视频消费 8 帧、`totalConsumedBytes=11059200`、sequence 1 到 17、`finalQueueDepth=2`，音频消费 40 个 PCM packet、`totalConsumedBytes=153600`、sequence 5 到 159、`finalQueueDepth=45`，两路 `finalConsumerStatus=manual-drain`、`exportedOverJson=false`。
  - interaction drain 阶段复测 `npm run media-worker:native:interaction-drain` 和 `npm run test:all:poc`：均通过，完整回归耗时约 59.9 秒；仍有 Vite chunk 体积超过 500 kB 警告。
