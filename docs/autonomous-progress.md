...
- 验证：
  ...
  - 执行 `npm run build`：通过，Workbench 已接入 Native Worker readiness 状态条；普通浏览器环境返回 `desktop-only`，不启动采集。
...
- Native Worker 控制面骨架：
  ...
  - 桌面端已新增 Native Worker readiness 诊断入口和工作台状态条；`probe_native_worker_devices` 只通过 Native Worker 执行 `listDevices` 枚举，不执行 `start`，不启动连续采集线程。
- 阻塞：
