...
- 验证：
  ...
  - WASAPI 阶段复测 `npm run media-worker:native:list-devices`：通过，当前 Windows 活跃设备为 1 路视频 `HD Webcam`、1 路音频采集 `麦克风阵列 (Senary Audio)` 和 1 路 WASAPI render 播放端点 `扬声器 (Senary Audio)`；该结果只反映当前接入状态，不覆盖前一次 4 路摄像头测试结果。
  ...
  - video format preference 阶段新增 `start.videoFormatPreference` 和脚本 `npm run media-worker:native:format-preference`：通过，先探测当前设备能力，再按偏好选择 NV12 1280x720@30；本机 `HD Webcam` inspected 17 个 media type，`selectedIndex=0`、`score=0`、线程复制 3 帧，证明采集线程使用了 channel 实际选中的 `mediaType.mediaTypeIndex`。
  - video channel binding 阶段新增 `start.videoChannelBindings` 和脚本 `npm run media-worker:native:channel-binding`：通过，`field-camera` 显式绑定到 `HD Webcam` 的 `index=0` / `deviceId=mf-video-0-4cc44571`，channel 返回 `deviceBinding.mode=explicit`，采集线程使用同一 deviceId 并复制 3 帧；未传绑定时仍按枚举顺序绑定。
  - native session plan 阶段新增 `npm run media-worker:native:session-plan`：通过，组合 `videoChannelBindings` 和 `videoFormatPreference` 启动短会话，生成 `native-worker/.tmp/session-plan-smoke.json`，schema 为 `smartst.native-session-plan.v0.1`，记录 `field-camera` 的 deviceBinding、mediaTypeSelection、device、mediaType 和 copiedFrames=3；该文件是硬件会话配置 smoke 摘要，不等同正式录像 manifest、数据库 schema 或 LiveKit room contract。
  - video preview drain 阶段新增 `npm run media-worker:native:video-preview-drain`：通过，2 秒内按 250ms 间隔模拟预览消费者 drain 1 帧，8 次 drain 均消费到帧，`totalConsumedFrames=8`、`totalConsumedBytes=11059200`、`firstSequence=1`、`lastSequence=17`、sequence 单调递增、`finalConsumerStatus=manual-drain`、`exportedOverJson=false`。
  - video PGM export 阶段新增 Native Worker 命令 `exportVideoPayloadQueuePgm` 和脚本 `npm run media-worker:native:video-pgm-export`：通过，1000ms 内当前 1 路 `HD Webcam` 复制 8 帧 NV12 payload，native-side 导出 1 帧 Y 平面为 `native-worker/.tmp/video-payload-export.pgm`，`width=1280`、`height=720`、`fileBytes=921616`、`luma.min=2`、`luma.max=32`、`consumerStatus=pgm-export`、`exportedOverJson=false`。
  - video PPM export 阶段新增 Native Worker 命令 `exportVideoPayloadQueuePpm` 和脚本 `npm run media-worker:native:video-ppm-export`：通过，1000ms 内当前 1 路 `HD Webcam` 复制 8 帧 NV12 payload，native-side CPU 转换并导出 1 帧 P6 RGB PPM，`width=1280`、`height=720`、`fileBytes=2764816`、`rgb.r.max=20`、`rgb.g.max=20`、`rgb.b.max=18`、`consumerStatus=ppm-export`、`exportedOverJson=false`。
  - 桌面控制面新增 Tauri `consume_native_worker_video_payload_queue` 命令、前端服务封装和工作台 `Drain video` 按钮；按钮只触发 native queue drain 并刷新 JSON 状态，不显示或传输帧字节。
  ...
  - 桌面控制面新增 Tauri `consume_native_worker_audio_payload_queue` 命令、前端服务封装和工作台 `Drain audio` 按钮；后续补充 `Drain AV` 联合按钮，顺序触发 1 帧视频 drain 和 5 个 PCM packet drain；这些按钮只 drain native queue 并刷新 JSON 状态，不显示或传输媒体 payload 字节。
  ...
  - audio call drain 阶段新增 `npm run media-worker:native:audio-call-drain`：通过，2 秒内按 250ms 间隔模拟通话或 publisher 消费者 drain 5 个 PCM packet，8 次 drain 均消费到 packet，`totalConsumedPackets=40`、`totalConsumedBytes=153600`、`firstSequence=5`、`lastSequence=160`、sequence 单调递增、`finalConsumerStatus=manual-drain`、`finalQueueDepth=45`、`audioLevelStatus=measured`、`exportedOverJson=false`。
  - audio call drain 阶段复测 `npm run media-worker:native:audio-call-drain` 和 `npm run test:all:poc`：均通过，完整回归耗时约 57.3 秒；仍有 Vite chunk 体积超过 500 kB 警告。
  - interaction drain 阶段新增 `npm run media-worker:native:interaction-drain`：通过，同一 Native Worker session 内启动 1 路视频和 1 路音频，2 秒内按 250ms 间隔同时 drain；视频消费 8 帧、`totalConsumedBytes=11059200`、sequence 1 到 17、`finalQueueDepth=2`，音频消费 40 个 PCM packet、`totalConsumedBytes=153600`、sequence 5 到 158、`finalQueueDepth=45`，两路 `finalConsumerStatus=manual-drain`、`exportedOverJson=false`。
  - interaction drain 阶段复测 `npm run media-worker:native:interaction-drain` 和 `npm run test:all:poc`：均通过，完整回归耗时约 60.3 秒；仍有 Vite chunk 体积超过 500 kB 警告。
  - 桌面控制面 `Drain AV` 阶段新增工作台联合 drain 按钮：通过，按钮在同一运行 session 内顺序调用 `consume_native_worker_video_payload_queue(maxFrames=1)` 和 `consume_native_worker_audio_payload_queue(maxPackets=5)` 并刷新状态；`npm run build` 和 `npm run test:all:poc` 均通过，完整回归耗时约 60.3 秒；该按钮仍只展示统计，不传输媒体 payload。
  - 桌面控制面 drain 可观测性增强：`Drain video`、`Drain audio` 和 `Drain AV` 后的 consumed 卡片新增 `status / seq / depth / frames|packets` 展示，便于确认 latestSequence 递增和 native queue 剩余深度；执行 `npm run build` 通过，仍有 Vite chunk 体积超过 500 kB 警告；仍不展示或传输媒体 payload。
  - 前端构建 code split 阶段完成：Workbench 对 `LiveKitPocPanel` 使用 lazy import，`VideoPane` 对 `hls.js` 使用按需 import，Vite 将 `livekit`、`hls`、`react-vendor`、`icons` 拆为独立 chunk；桌面端主工作台 chunk 约 46.29 kB，`livekit` chunk 约 503.66 kB，`hls` chunk 约 524.95 kB，web-observer PoC JS 约 508.53 kB；桌面端和 web-observer 构建均无 Vite chunk warning。
  - 业务服务默认画面合同阶段完成：`server-poc` 在 `accept call` 和 `room create` 中生成 `mediaPolicy.defaultChannelId/defaultTrackName/defaultSelectionReason/startupVideoMode`，选择顺序为手动指定、本地主画面、远端默认、优先级、audio-only；token response metadata 和真实 LiveKit JWT metadata 均写入默认画面字段；`npm run server:poc:smoke` 和 `npm run server:poc:real-token-smoke` 已验证手动默认、local-primary 优先、priority fallback、audio-only、非法默认通道拒绝和手机 H5 subscribe-only 权限。
  - Native AV soak 阶段新增 `npm run media-worker:native:av-soak`：默认 5 秒同时启动视频/音频采集，500ms 周期采样并 drain native queue，写入 `native-worker/.tmp/av-soak-smoke.json`，schema 为 `smartst.native-av-soak.v0.1`；完整回归内通过，10 次采样/10 次 drain，视频和音频 copy counters 均增长，音视频均完成 native queue 消费，两路 queue depth 均未超过配置容量、copy error 为 0、`exportedOverJson=false`；`npm run media-worker:native:payload-consume` 现已断言 requested `channelId` 与返回 `channelId` 一致，避免多通道场景退回隐式第一路 drain；`npm run test:all:poc` 完整回归通过，耗时约 67.1 秒。
  - audio WAV export 阶段新增 Native Worker 命令 `exportAudioPayloadQueueWav` 和脚本 `npm run media-worker:native:audio-wav-export`：通过，1000ms 内当前 1 路 WASAPI 音频复制 96 个 PCM packet，native-side 导出 10 个 packet 到 `native-worker/.tmp/audio-payload-export.wav`，`exportedBytes=38400`、`fileBytes=38444`、WAV header 为 IEEE_FLOAT/2ch/48000Hz/32-bit、`consumerStatus=wav-export`、`exportedOverJson=false`。
  - native export artifact manifest 阶段新增共享解析工具 `native-worker/export-artifact-utils.mjs` 和脚本 `npm run media-worker:native:export-artifact-manifest`：通过，读取已导出的 PGM/PPM/WAV，默认要求产物 mtime 在 300000ms 内，生成 `native-worker/.tmp/export-artifact-manifest.json`，记录 3 个 artifact 的 bytes、SHA-256 和格式统计；该 manifest 是 Native Worker 导出产物完整性清单，不等同正式录像 manifest、患者绑定合同或播放索引。
  - audio WAV export 阶段复测 `cargo build --manifest-path native-worker/Cargo.toml` 和 `npm run test:all:poc`：均通过，完整回归耗时约 44.8 秒；`native-worker/.tmp/` 已加入 `.gitignore`，仍有 Vite chunk 体积超过 500 kB 警告。
  ...
  - WASAPI loopback capture 初始化阶段新增 Native Worker 命令 `captureAudioLoopbackBuffer` 和脚本 `npm run media-worker:native:audio-loopback-probe`：通过，500ms 内当前第 0 路 render 端点 `扬声器 (Senary Audio)` 以 `AUDCLNT_STREAMFLAGS_LOOPBACK` 打开 capture client 并停止，当前安静系统返回 `status=no-loopback-packets`、`packetCount=0`、`capturedFrames=0`、`loopbackClientStatus=opened-stopped`、`audibleOutputGenerated=false`、`aecStatus=not-run`；`npm run test:all:poc` 完整回归通过，耗时约 69.1 秒；该结果证明 loopback client 生命周期可用，不证明已捕获回放内容或已实现 AEC。
...
- Native Worker 控制面骨架：
  ...
  - `listDevices` 已接入 Windows 原生枚举：Media Foundation 视频设备、WASAPI/Core Audio 采集端点和 WASAPI render 播放端点；`probeAudioRenderFormat` 可读取 render endpoint 的 mix format 和 device period；`renderAudioSilence` 可短时打开 render client 并写入 silent buffer；`captureAudioLoopbackBuffer` 可短时打开 render endpoint loopback capture client。
  ...
  - 已增加 `probeAudioFormat`、`probeAudioRenderFormat`、`captureAudioBuffer`、`renderAudioSilence` 和 `captureAudioLoopbackBuffer`，可验证 WASAPI capture/render mix format、短时 capture buffer 读取、render client 静音写入和 loopback capture client 初始化。
  ...
  - 桌面发布打包阶段完成：`tauri:build:exe` 和 `tauri:build` 现会先构建 `native-worker/target/release/smartst-native-worker.exe`；Tauri `bundle.resources` 将 release Worker 安装为 `bin/smartst-native-worker.exe`；Tauri 后端启动顺序调整为 packaged Worker 优先、workspace debug Worker 次之、Cargo source fallback 最后；`cargo test --manifest-path src-tauri/Cargo.toml`、`npm run build`、`npm run tauri:build:exe`、`npm run tauri:build`、`npm run test:all:poc` 均通过，完整 PoC 回归耗时约 70.4 秒，NSIS 脚本确认包含 Worker 安装和卸载条目。
- 阻塞：
