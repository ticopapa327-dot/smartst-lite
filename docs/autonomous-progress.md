...
- 验证：
  ...
  - video PGM export 阶段新增 Native Worker 命令 `exportVideoPayloadQueuePgm` 和脚本 `npm run media-worker:native:video-pgm-export`：通过，1000ms 内当前 1 路 `HD Webcam` 复制 8 帧 NV12 payload，native-side 导出 1 帧 Y 平面为 `native-worker/.tmp/video-payload-export.pgm`，`width=1280`、`height=720`、`fileBytes=921616`、`luma.min=2`、`luma.max=36`、`consumerStatus=pgm-export`、`exportedOverJson=false`。
  - video PPM export 阶段新增 Native Worker 命令 `exportVideoPayloadQueuePpm` 和脚本 `npm run media-worker:native:video-ppm-export`：通过，1000ms 内当前 1 路 `HD Webcam` 复制 8 帧 NV12 payload，native-side CPU 转换并导出 1 帧 P6 RGB PPM，`width=1280`、`height=720`、`fileBytes=2764816`、`rgb.r.max=21`、`rgb.g.max=21`、`rgb.b.max=21`、`consumerStatus=ppm-export`、`exportedOverJson=false`。
  ...
  - audio WAV export 阶段新增 Native Worker 命令 `exportAudioPayloadQueueWav` 和脚本 `npm run media-worker:native:audio-wav-export`：通过，1000ms 内当前 1 路 WASAPI 音频复制 94 个 PCM packet，native-side 导出 10 个 packet 到 `native-worker/.tmp/audio-payload-export.wav`，`exportedBytes=38400`、`fileBytes=38444`、WAV header 为 IEEE_FLOAT/2ch/48000Hz/32-bit、`consumerStatus=wav-export`、`exportedOverJson=false`。
