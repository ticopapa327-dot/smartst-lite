...
- 验证：
  ...
  - video PGM export 阶段新增 Native Worker 命令 `exportVideoPayloadQueuePgm` 和脚本 `npm run media-worker:native:video-pgm-export`：通过，1000ms 内当前 1 路 `HD Webcam` 复制 8 帧 NV12 payload，native-side 导出 1 帧 Y 平面为 `native-worker/.tmp/video-payload-export.pgm`，`width=1280`、`height=720`、`fileBytes=921616`、`luma.min=1`、`luma.max=28`、`consumerStatus=pgm-export`、`exportedOverJson=false`。
  - video PPM export 阶段新增 Native Worker 命令 `exportVideoPayloadQueuePpm` 和脚本 `npm run media-worker:native:video-ppm-export`：通过，1000ms 内当前 1 路 `HD Webcam` 复制 8 帧 NV12 payload，native-side CPU 转换并导出 1 帧 P6 RGB PPM，`width=1280`、`height=720`、`fileBytes=2764816`、`rgb.r.max=25`、`rgb.g.max=23`、`rgb.b.max=20`、`consumerStatus=ppm-export`、`exportedOverJson=false`。
  ...
  - audio WAV export 阶段新增 Native Worker 命令 `exportAudioPayloadQueueWav` 和脚本 `npm run media-worker:native:audio-wav-export`：通过，1000ms 内当前 1 路 WASAPI 音频复制 97 个 PCM packet，native-side 导出 10 个 packet 到 `native-worker/.tmp/audio-payload-export.wav`，`exportedBytes=38400`、`fileBytes=38444`、WAV header 为 IEEE_FLOAT/2ch/48000Hz/32-bit、`consumerStatus=wav-export`、`exportedOverJson=false`。
...
- Native Worker 控制面骨架：
  ...
  - 当前已接入多路视频线程结构、Native Worker 显式视频通道绑定、Native Worker 视频格式偏好选择、native-only 有界帧 payload 队列、视频手动 drain 消费验证、视频 native-side NV12/PGM 灰度导出、NV12/PPM RGB 转换导出验证、Native Worker 导出产物 manifest 校验、Tauri/工作台 Drain video 控制、WASAPI RMS/peak 音量统计、native-only 有界 PCM packet payload 队列、音频手动 drain 消费验证、音频 native-side WAV 文件导出验证和 stop/join 清理，但本轮本机只枚举到 1 路视频设备；尚未接入预览纹理、音频重采样/AEC、LiveKit native publisher 或真实录像。
