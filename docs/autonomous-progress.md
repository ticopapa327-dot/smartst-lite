...
- Native Worker 控制面骨架：
  ...
  - 桌面端已新增 Native Worker readiness 诊断入口、工作台状态条、手动 `Device Probe` 面板、手动 start/status/stop 控件、`Drain video`、`Drain audio` 和 `Drain AV` 控件；`probe_native_worker_devices` 只通过 Native Worker 执行 `listDevices` 枚举，不执行 `start`，不启动连续采集线程；start/status/stop/drain 控件只展示 JSON 状态统计，不传输媒体 payload。
