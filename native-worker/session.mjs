...
child.on("close", (exitCode) => {
  if (completed) return;
  const error = new Error(`native worker exited before session probe completed. exit=${exitCode}. stderr=${stderr}`);
  rejectReady(error);
...
try {
  ...
  completed = true;
  console.log(JSON.stringify({ started, status, stopped }, null, 2));
