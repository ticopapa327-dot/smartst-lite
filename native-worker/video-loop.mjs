...
const mediaTypeIndex = readIntegerEnv("UST_NATIVE_VIDEO_MEDIA_TYPE_INDEX", 0);
const durationMs = readIntegerEnv("UST_NATIVE_VIDEO_DURATION_MS", 2000);
const maxReads = readIntegerEnv("UST_NATIVE_VIDEO_MAX_READS", 10000);
...
child.on("close", (exitCode) => {
  if (completed) return;
  const error = new Error(`native worker exited before video loop completed. exit=${exitCode}. stderr=${stderr}`);
  rejectReady(error);
...
try {
  ...
  const measurement = await request("measureVideoFrames", params, Math.max(30000, durationMs + 15000));
  completed = true;
