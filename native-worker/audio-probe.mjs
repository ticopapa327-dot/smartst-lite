...
const durationMs = readIntegerEnv("SMARTST_NATIVE_AUDIO_DURATION_MS", 500);
const pollIntervalMs = readIntegerEnv("SMARTST_NATIVE_AUDIO_POLL_INTERVAL_MS", 10);
...
child.on("close", (exitCode) => {
  if (completed) return;
  const error = new Error(`native worker exited before audio probe completed. exit=${exitCode}. stderr=${stderr}`);
  rejectReady(error);
...
try {
  ...
  const format = await request("probeAudioFormat", params);
  ...
  completed = true;
  console.log(JSON.stringify({ format, capture }, null, 2));
