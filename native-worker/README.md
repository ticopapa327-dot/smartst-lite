...
- Channel `start`/`stop`/`status` now exposes a native capture session skeleton with device binding metadata.
...
`start` now binds requested channels to currently available Media Foundation devices by index and binds one WASAPI capture endpoint for session metadata. Missing video devices are reported as `waiting-for-device`; they do not block the worker from starting. Continuous capture threads are still not started by `start`.
